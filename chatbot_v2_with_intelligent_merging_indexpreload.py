...
# ✅ Retrieve FAISS Index (Pre-loading strategy)
def get_faiss_retriever():
    global vectorstore
    if vectorstore is None:
        print("⚠️ FAISS index was not preloaded! Initializing now...")
...
1️⃣ First, try to answer using the documents above.
2️⃣ If the answer is **not fully available** in the documents, you may use your own knowledge.
3️⃣ If both sources provide useful information, combine them clearly.
...
# ✅ Define Intelligent Hybrid RAG Chain
def get_response_chain_new(answer_mode):
    retriever = get_faiss_retriever()

    if answer_mode == "LLM_ONLY":
        return RunnableLambda(lambda query: llm.invoke(query["question"]))

    elif answer_mode == "KB_ONLY":
        return (
                RunnableParallel({
                    "context": lambda x: retriever.invoke(x) if retriever else "No relevant documents.",
        ...
        def retrieve_and_merge(query):
            retrieved_docs = retriever.invoke(query) if retriever else []
            retrieved_text = "\n\n".join(doc.page_content for doc in retrieved_docs)

            llm_response = llm.invoke(query) if not retrieved_docs else None

            if retrieved_text and llm_response:
                return f"{retrieved_text}\n\nAdditionally, based on general knowledge:\n{llm_response}"
            return retrieved_text or llm_response

        return (
                RunnableParallel({
                    "context": RunnableLambda(lambda q: retrieve_and_merge(q["question"])),
...
def preload_faiss_index():
    global vectorstore
    print(f"📌 Preloading FAISS Index at {datetime.now().strftime('%H:%M:%S')}...")

    documents = load_documents()
    vectorstore = create_or_update_vectorstore(documents)

    if vectorstore:
        print(f"✅ FAISS Index Loaded Successfully! at {datetime.now().strftime('%H:%M:%S')}...")
    else:
        print(f"⚠️ FAISS Index is empty or not loaded properly!")
